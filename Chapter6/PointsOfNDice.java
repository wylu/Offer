...
public class PointsOfNDice {
    private void printProbability(int num) {
        ...
        //骰子最大面值
        int diceMaxVal = 6;
        int[][] prob = new int[2][diceMaxVal * num + 1];

        int flag = 0;
        for (int i = 1; i <= diceMaxVal; i++) prob[flag][i] = 1;

        for (int k = 2; k <= num; k++) {
            //有k个骰子时，最小点数和为k，不存在出现点数和小于k的情况
            for (int i = 0; i < k; i++) prob[1 - flag][i] = 0;
        ...
        for (int i = num; i <= diceMaxVal * num; i++) {
            System.out.println(String.format("%d : %.2f", i, prob[flag][i] / total));
        }
    }

    public static void main(String[] args) {
        new PointsOfNDice().printProbability(2);
    }
}
